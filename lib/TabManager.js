var TabManager = React.createFactory(
  React.createClass({
      ...
      return React.DOM.div(
        ...
        this.state.windows.map((window) => {
          return Window({
            ...
            delete: this.deleteTab.bind(this),
    ...
    deleteTabs: function () {
      var tabs = Object.keys(this.state.selection).map(
        (id) => this.state.tabsbyid[id]
      );
      if (tabs.length) {
    ...
    drop: function (id, before) {
      ...
      for (var i = 0; i < tabs.length; i++) {
        (function (t) {
          chrome.tabs.move(
            t.id,
            { windowId: tab.windowId, index: index },
            function () {
              chrome.tabs.update(t.id, { pinned: t.pinned });
            }
          );
