var Window = React.createFactory(
  React.createClass({
    render: function () {
      var hideWindow = true;
      var tabsperrow =
      ...
      if (!hideWindow) {
        tabs.push(
          React.DOM.div({
            className:
              "icon add " +
              (this.props.layout == "blocks" ? "" : "windowaction"),
            onClick: this.addTab,
          })
        );
        tabs.push(
        ...
        for (var j = 0; j < tabs.length; j++) {
          if (
            j % tabsperrow == 0 &&
            j &&
            (j < tabs.length - 1 || this.props.layout == "blocks")
          ) {
        ...
        return React.DOM.div(
          {
            className:
              "window " +
              (this.props.layout == "blocks" ? "block" : "") +
              (this.props.window.focused ? "focused" : ""),
          },
          children
        );
      } else {
